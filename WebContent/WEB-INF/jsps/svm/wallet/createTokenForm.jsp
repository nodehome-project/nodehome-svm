...
<html>
  ...
  <link rel="stylesheet" href="https://use.fontawesome.com/releases/v5.4.2/css/all.css" integrity="sha384-/rXc/GQVaYpyDdyxK+ecHPVYJSN9bmVFBvjA/9eOB+pb3F2w2N6fc5qB9Ew5yIns" crossorigin="anonymous">
	...
	function FnCreateToken() {
		if(myBalance < parseInt(policyData['FeeCreateToken'])) {
			$.alert('잔액이 부족합니다.');
			return false;
		}
		...
		setTimeout(function() {
	 		...
	 		var tokenName = frm.tokenName.value;
	 		var tokenUnit = frm.tokenUnit.value;
	 		var memo = frm.memo.value;

			var sArgs = ["PID","10000",tokenName, tokenUnit, memo];
			var displayParam = [];
			AWI_runTransaction("createToken", sArgs, "FnCallBackFunction", "Create Token", "", displayParam, j_curWID, parseInt(policyData['FeeCreateToken']));
	...
	function FnCallBackFunction(strJson) {
		...
		if(joFunc['result'] == "OK") {
			var rtnObjValue = joFunc['value'];
			var rtnObjValue2 = rtnObjValue['value'];
			var token_id = rtnObjValue2['token_id'];
			$.alert({
			    title: '안내',
			    content: "Token생성이 완료 되었습니다.<br/>token id : "+token_id+"<br/>Token 발행 화면으로 이동합니다.",
			    confirm: function(){
			    },
			    onClose: function(){
					frm.action="/svm/wallet/mintTokenForm?token_id="+token_id;
					frm.submit();
			    },
			});
	...
	<div class="container">
		...
		<div class="row">
		    <div class="col-xs-12 col-sm-8 col-md-6 col-sm-offset-2 col-md-offset-3">
				...
				<h2>Create Token <br /><small></small></h2>
				<hr class="colorgraph">
					
	            <div class="form">
	                <div class="form-row">
	                  <div class="form-group col-lg-6">
	                    <input type="text" class="form-control" name="tokenName" id="tokenName" placeholder="Token Name" />
	                  </div>
	                  <div class="form-group col-lg-6">
	                    <input type="text" class="form-control" name="tokenUnit" id="tokenUnit" placeholder="Token Unit"  />
	                  </div>
	                </div>
					...
					<div style="" class="row">
						<div class="col-6" style="padding-right:3px;"><div style="padding:2px 5px 2px 5px;background-color:#4A76D9;border-radius:15px;color:#fff;text-align:center;" onclick="FnCreateToken();"><spring:message code="user.text.create"/></div></div>
